...
def test_vote_restaurant(authorized_client: TestClient, test_restaurant: Restaurant) -> None:
    response = authorized_client.post(f"{settings.API_V1_STR}/restaurants/{test_restaurant.id}/vote")
...
def test_vote_limit(authorized_client: TestClient, db: Session, test_restaurant: Restaurant) -> None:
    ...
    for idx, restaurant_id in enumerate(restaurant_ids):
        response = authorized_client.post(f"{settings.API_V1_STR}/restaurants/{restaurant_id}/vote")
...
def test_vote_weights(authorized_client: TestClient, db: Session, test_restaurant: Restaurant) -> None:
    # Create another user
    user2 = User(
...
def test_vote_history(authorized_client: TestClient, db: Session, test_restaurant: Restaurant, test_dates: tuple[date, date]) -> None:
    # Add some votes
    for _ in range(3):
        authorized_client.post(f"{settings.API_V1_STR}/restaurants/{test_restaurant.id}/vote")
...
def test_winner_determination(authorized_client: TestClient, db: Session) -> None:
    ...
    for _ in range(3):
        authorized_client.post(f"{settings.API_V1_STR}/restaurants/{restaurant1.id}/vote")
...
def test_unauthorized_vote(client: TestClient, test_restaurant: Restaurant) -> None:
    response = client.post(f"{settings.API_V1_STR}/restaurants/{test_restaurant.id}/vote")
