...
def test_create_duplicate_restaurant(authorized_client: TestClient, test_restaurant: Restaurant) -> None:
    data = {
...
def test_get_restaurants(authorized_client: TestClient, test_restaurant: Restaurant) -> None:
...
def test_get_restaurant(authorized_client: TestClient, test_restaurant: Restaurant) -> None:
    response = authorized_client.get(f"{settings.API_V1_STR}/restaurants/{test_restaurant.id}")
    assert response.status_code == 200
    content = response.json()
...
def test_update_restaurant(authorized_client: TestClient, test_restaurant: Restaurant) -> None:
    data = {
...
def test_delete_restaurant(authorized_client: TestClient, test_restaurant: Restaurant) -> None:
    response = authorized_client.delete(f"{settings.API_V1_STR}/restaurants/{test_restaurant.id}")
    assert response.status_code == 200
    content = response.json()
    assert content["id"] == test_restaurant.id
